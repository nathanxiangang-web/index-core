...
| T-C9 | `pending_not_before` handled per state | a merge into `PENDING` may make it eligible sooner; a merge into `RETRY_WAIT` leaves `pending_not_before` **unchanged** (backoff not cancelled); a new epoch (`VERIFIED`) rebuilds `pending_*`; same-group `pending_priority` = **max** of the un-claimed signals. |
