...
| T-F8 | counter ownership | a merged `POLL_SCHEDULE` + `MUTATION_HINT` attempt that fails updates **Work** counters; **Watch** counters/attempt fields update **only** when `POLL_SCHEDULE ∈ source_set`; budget defer updates neither. || T-C9 | `not_before` handled per state | a merge into `PENDING` may make it eligible sooner; a merge into `RETRY_WAIT` leaves `not_before` **unchanged** (backoff not cancelled); a new epoch (`VERIFIED`) rebuilds `priority_class`/`first_seen_at`/`last_seen_at`/`not_before`. |
