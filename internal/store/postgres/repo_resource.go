...
// InsertCanonicalResource inserts T3. No unique path constraint (C-C3 REJECTED).
func (s *Store) InsertCanonicalResource(ctx context.Context, q Querier, r domain.CanonicalResource) error {
	_, err := q.Exec(ctx,
		`INSERT INTO index_canonical_resource(
		     ...
		     missing_first_snapshot_id, missing_last_snapshot_id, canonical_path, parent_resource_id,
		     name, is_dir, size, mtime, content_hash, hash_algorithm, content_type, current_attributes)
		 VALUES ($1::uuid, $2::uuid, $3, $4, $5, $6, $7, $8, $9::uuid, $10::uuid, $11, $12::uuid,
		...
		r.MissingFirstSnapshotID, r.MissingLastSnapshotID, r.CanonicalPath, r.ParentResourceID,
		r.Name, r.IsDir, r.Size, r.Mtime, r.ContentHash, r.HashAlgorithm, r.ContentType, r.CurrentAttributes)
	return err
}
...
func (s *Store) queryCanonical(ctx context.Context, q Querier, where string, args ...any) ([]domain.CanonicalResource, error) {
	rows, err := q.Query(ctx,
		`SELECT resource_id::text, root_id::text, introduced_at_generation, last_confirmed_generation,
		        resource_presence, removal_evidence_state, missing_since, consecutive_complete_missing,
		        missing_first_snapshot_id::text, missing_last_snapshot_id::text, canonical_path,
		        parent_resource_id::text, name, is_dir, size, mtime,
...
func (s *Store) SetRemovalEvidence(ctx context.Context, q Querier, resourceID string, state domain.RemovalEvidenceState, missingSince any, consecutive int32, missingFirstSnapshotID, missingLastSnapshotID any, lastConfirmed int64) error {
	_, err := q.Exec(ctx,
		`UPDATE index_canonical_resource
		    SET removal_evidence_state = $2, missing_since = $3,
		        ...
		        missing_last_snapshot_id = $6, last_confirmed_generation = $7, updated_at = now()
		  WHERE resource_id = $1::uuid`,
		resourceID, string(state), missingSince, consecutive, missingFirstSnapshotID, missingLastSnapshotID, lastConfirmed)
	return err
}
	...
	_, err := q.Exec(ctx,
		`UPDATE index_canonical_resource
		    SET removal_evidence_state = 'NONE', missing_since = NULL,
		        ...
		        missing_last_snapshot_id = NULL, last_confirmed_generation = $2, updated_at = now()
...
// PresentResourcesAtPath returns PRESENT resources sharing a canonical path.
